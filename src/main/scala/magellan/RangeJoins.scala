...
object RangeJoins {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("Magellan Range Joins")
    conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    conf.setMaster("local[*]")

    val spark = new SparkContext(conf)
    val sparkSession = SparkSession.builder().appName("Magellan Range Joins").getOrCreate()
    ...
    val buildsPath = homePath + "buildings_1M.csv"
    val partNum=50
    //    pointPoint()
    //    pointLineString()
    //    pointRectangle()
    ...
    def polygonPolygon(parksPath:String, lakesPath:String,partNum:Int) {
      ...
      val bufferRange = 0.1
      val bufferBegin = System.currentTimeMillis()
      val bufferedIndexedParks = indexedParks.withColumn("buffered", $"polygon" buffer bufferRange)
      val bufferedIndexedLakes = indexedLakes.withColumn("buffered", $"polygon" buffer bufferRange)
      val bufferTime= System.currentTimeMillis() - bufferBegin

      println("Buffering Time: " + (bufferTime) / 1E3 + " sec ")

      t0 = System.nanoTime()
      count = indexedLakes.join(bufferedIndexedParks, bufferedIndexedParks("curve") === bufferedIndexedLakes("curve")).where(bufferedIndexedLakes("relation") === "Intersects").count()
      t1 = System.nanoTime()
      val time0 = ((t1 - t0) / 1E9)
      println("Join Time: " + time0 + " sec")
