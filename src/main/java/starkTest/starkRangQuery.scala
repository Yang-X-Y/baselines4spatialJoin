...
object starkRangQuery {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val joinResPlain = rangeQuery(indexRDDPartitionedIndex, brodeCast)
  ...
  def rangeQuery(indexRDD: LiveIndexedSpatialRDDFunctions[STObject, Int], brodeCast: Broadcast[Array[STObject]]): Long = {
    var num: Long = 0
    val queryRDD = brodeCast.value
    println("queryRDD size: " + queryRDD.length)
    for (q <- queryRDD) {
      val res = indexRDD.intersects(q)
      num += res.count()
    }
    num
  }
