...
object starkKnnQuery {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val joinResPlain = knnQuery(indexRDDPartitionedIndex, brodeCast, k)
  ...
  def knnQuery(indexRDD: LiveIndexedSpatialRDDFunctions[STObject, Int], brodeCast: Broadcast[Array[STObject]], k: Int): Long = {

    var num: Long = 0
    val queryRDD = brodeCast.value
    println("queryRDD size: " + queryRDD.length)
    for (q <- queryRDD) {
      val res = indexRDD.kNN(q, k, Distance.seuclid)
